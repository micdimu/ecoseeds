...
Germination.Analysis<-function(germ_dyn, Nv.seed=NULL, cv=1.5, n.seed=20){
  ...
  Dyn_perc_germ<-aggregate(Dyn_perc_germ_SP, by=list(test_type), FUN=mean, na.action = na.omit)[,-1]

  M<-aggregate(M_SP, by=list(test_type), FUN=mean, na.action = na.omit)[,-1]
  names(M)<-levels(test_type)

  M_SD<-aggregate(M_SP, by=list(test_type), FUN=sd, na.action = na.omit)[,-1]
  names(M)<-levels(test_type)

  Z<-aggregate(Z_SP, by=list(test_type), FUN=mean, na.action = na.omit)[,-1]
  names(Z)<-levels(test_type)

  Z_SD<-aggregate(Z_SP, by=list(test_type), FUN=sd, na.action = na.omit)[,-1]
  names(Z)<-levels(test_type)
  ...
  T50<-aggregate(T50_SP, by=list(test_type), FUN=mean, na.action = na.omit)[,-1]
  k<-aggregate(k_SP, by=list(test_type), FUN=mean, na.action = na.omit)[,-1]

  T50_SD<-aggregate(T50_SP, by=list(test_type), FUN=sd, na.action = na.omit)[,-1]
  k_SD<-aggregate(k_SP, by=list(test_type), FUN=sd, na.action = na.omit)[,-1]
