Species.T50.comp<-function(Germ.Analysis.exp_sp1, Germ.Analysis.exp_sp2, sp_name=NULL , colour="yes", Test.int=NULL){
  require(plyr)
  require(agricolae)
  require(gdata)
  require(ggplot2)
  ...
  T50_M0_sp1<-Germ.Analysis.exp_sp1$T50_SP[sel_M0_sp1]
  T50_M0_sp2<-Germ.Analysis.exp_sp2$T50_SP[sel_M0_sp2]
  ...
  comm_T50_sp1<-T50_M0_sp1[which(sel_comm_test_sp1==TRUE)]
  comm_T50_sp2<-T50_M0_sp2[which(sel_comm_test_sp2==TRUE)]

  comm_sp1<-cbind.data.frame(T50=comm_T50_sp1,  test=comm_test_sp1)
  comm_sp2<-cbind.data.frame(T50=comm_T50_sp2,  test=comm_test_sp2)
  ...
  A1<-aov(T50 ~ test_sp, data=comm_sp1_sp2)
  a<-HSD.test(A1, 'test_sp')

  sign <- a$groups[order(ordered(row.names(a$groups), levels(comm_sp1_sp2$test_sp))),]
  ...
  Boxplot_com_spe<-ggplot(comm_sp1_sp2, aes(x=Legend1, y = T50)) +
    geom_boxplot(aes(group=Legend1, y = T50, col=Legend))+
    geom_text(data=sign_t, aes(x=c(1:nlevels(Legend1)), label = sign,
                               y=aggregate(comm_sp1_sp2$T50, by=list(Legend1), max)[,2]+5),size=5)+
    theme(axis.text.x = element_text(angle = 45, hjust = 1))+
    scale_y_continuous("T50") +
    ...
    Boxplot_com_spe<-ggplot(comm_sp1_sp2, aes(x=Legend1, y = T50)) +
      geom_boxplot(aes(group=Legend1, y = T50))+
      geom_text(data=sign_t, aes(x=c(1:nlevels(Legend1)), label = sign,
                                 y=aggregate(comm_sp1_sp2$T50, by=list(Legend1), max)[,2]+5),size=5)+
      theme(axis.text.x = element_text(angle = 45, hjust = 1))+
      scale_y_continuous("T50") +
  ...
  ggsave("Boxplot_compare_species_T50.tiff", plot = Boxplot_com_spe, width = 174, height = 98, units = c("mm"),dpi = 300)
  plot(Boxplot_com_spe)


  Com.spe.G.V<-list(summary(A1), a, levels(comm_sp1_sp2$test), comm_sp1_sp2$T50, Boxplot_com_spe)
  names(Com.spe.G.V)=c("Anova", "Tukey", "Test", "T50", "Boxplot_com_spe_T50")
