Species.FinGerm.comp<-function(Germ.Analysis.exp_sp1, Germ.Analysis.exp_sp2, sp_name=NULL , colour="yes", Test.int=NULL){
  require(plyr)
  require(agricolae)
  require(gdata)
  require(ggplot2)
  ...
  comm_test_sp1<-Germ.Analysis.exp_sp1$test_type[which(sel_comm_test_sp1==TRUE)]
  comm_test_sp2<-Germ.Analysis.exp_sp2$test_type[which(sel_comm_test_sp2==TRUE)]

  comm_M_sp1<-Germ.Analysis.exp_sp1$M_SP[which(sel_comm_test_sp1==TRUE)]
  comm_M_sp2<-Germ.Analysis.exp_sp2$M_SP[which(sel_comm_test_sp2==TRUE)]

  comm_sp1<-cbind.data.frame(M=comm_M_sp1,  test=comm_test_sp1)
  comm_sp2<-cbind.data.frame(M=comm_M_sp2,  test=comm_test_sp2)
  ...
  A1<-aov(M ~ test_sp, data=comm_sp1_sp2)
  ...
  Boxplot_com_spe<-ggplot(comm_sp1_sp2, aes(x=Legend1, y = M)) +
    ...
    Boxplot_com_spe<-ggplot(comm_sp1_sp2, aes(x=Legend1, y = M)) +
      geom_boxplot(aes(group=Legend1, y = M))+
      geom_text(data=sign_t, aes(x=c(1:nlevels(Legend1)), label = sign,
                y=aggregate(comm_sp1_sp2$M, by=list(Legend1), max)[,2]+5),size=5)+
      theme(axis.text.x = element_text(angle = 45, hjust = 1))+
      scale_y_continuous("Final Germination %") +
  ...
  ggsave("Boxplot_compare_species_FinGerm.tiff", plot = Boxplot_com_spe, width = 174, height = 98, units = c("mm"),dpi = 300)
  plot(Boxplot_com_spe)


  Com.spe.G.V<-list(summary(A1), a, levels(comm_sp1_sp2$test), comm_sp1_sp2$M, Boxplot_com_spe)
  names(Com.spe.G.V)=c("Anova", "Tukey", "Test", "FinGerm", "Boxplot_com_spe_FinGerm")
